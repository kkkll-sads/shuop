...
const OrderSection: React.FC<OrderSectionProps> = ({ title, items, onItemClick }) => (
  <div className="bg-white rounded-xl p-4 mb-4 shadow-sm">
    <div className="flex items-center mb-4 border-l-4 border-blue-600 pl-2">
    ...
    <div className={`grid grid-cols-${items.length > 4 ? 4 : items.length} gap-4`}>
      {items.map((item, idx) => (
        <div 
          key={idx} 
          className="flex flex-col items-center active:opacity-60 cursor-pointer transition-opacity group"
          onClick={() => onItemClick(item.actionKey)}
        >
          <div className="relative mb-2 p-2 rounded-full bg-blue-50 group-hover:bg-blue-100 transition-colors">
             <item.icon size={24} className="text-blue-600" strokeWidth={1.5} />
          </div>
